...
export function login () {
  ...
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    const email = typeof req.body.email === 'string' ? req.body.email : '' // vuln-code-snippet hide-line
    models.sequelize.query('SELECT * FROM Users WHERE email = $email AND password = $password AND deletedAt IS NULL', { bind: { email, password: security.hash(req.body.password || '') }, model: UserModel, plain: true }) // vuln-code-snippet vuln-line loginAdminChallenge loginBenderChallenge loginJimChallenge
