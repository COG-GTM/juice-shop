...
export function login () {
  ...
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    models.sequelize.query('SELECT * FROM Users WHERE email = $email AND password = $password AND deletedAt IS NULL', { bind: { email: String(req.body.email || ''), password: security.hash(String(req.body.password || '')) }, model: UserModel, plain: true }) // vuln-code-snippet vuln-line loginAdminChallenge loginBenderChallenge loginJimChallenge
