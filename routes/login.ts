...
export function login () {
  ...
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    models.sequelize.query('SELECT * FROM Users WHERE email = $mail AND deletedAt IS NULL', { bind: { mail: String(req.body.email ?? '') }, model: UserModel, plain: true }) // vuln-code-snippet vuln-line loginAdminChallenge loginBenderChallenge loginJimChallenge
      .then((authenticatedUser) => { // vuln-code-snippet neutral-line loginAdminChallenge loginBenderChallenge loginJimChallenge
        const user = utils.queryResultToJson(authenticatedUser)
        if (!security.verifyPassword(String(req.body.password ?? ''), user.data?.password)) { // vuln-code-snippet hide-line
          res.status(401).send(res.__('Invalid email or password.')) // vuln-code-snippet hide-line
          return // vuln-code-snippet hide-line
