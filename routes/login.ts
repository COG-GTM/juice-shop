...
export function login () {
  ...
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    models.sequelize.query('SELECT * FROM Users WHERE email = $mail AND password = $pass AND deletedAt IS NULL', { bind: { mail: String(req.body.email ?? ''), pass: security.hash(String(req.body.password ?? '')) }, model: UserModel, plain: true }) // vuln-code-snippet vuln-line loginAdminChallenge loginBenderChallenge loginJimChallenge
