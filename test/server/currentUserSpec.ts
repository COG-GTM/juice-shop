...
describe('currentUser', () => {
  ...
  it('should return ID and email of user belonging to cookie from the request', () => {
    const user = { id: 1, email: 'admin@juice-sh.op', lastLoginIp: '0.0.0.0', profileImage: '/assets/public/images/uploads/default.svg' }
    const token = authorize({ data: user })
    req.cookies.token = token
    req.query.callback = undefined
    authenticatedUsers.put(token, { data: user as unknown as UserModel })
