...
void describe('/rest/products/search', () => {
  ...
  void it('GET product search with injection-like semicolon is treated as a literal search', async () => {
    const res = await request(app)
  ...
  void it('GET product search with injection-like union select is treated as a literal search', async () => {
    const res = await request(app)
  ...
  void it('GET product search with injection-like union select and one parenthesis is treated as a literal search', async () => {
    const res = await request(app)
      .get("/rest/products/search?q=') union select id,email,password from users--")
    assert.equal(res.status, 200)
    assert.ok(res.headers['content-type']?.includes('application/json'))
  ...
  void it('GET product search with injection-like union select star is treated as a literal search', async () => {
    const res = await request(app)
  ...
  void it('GET product search cannot select logically deleted christmas special via injection-like comment payload', async () => {
    const res = await request(app)
  ...
  void it('GET product search cannot select logically deleted unsafe product via injection-like comment payload', async () => {
    const res = await request(app)
