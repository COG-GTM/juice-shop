...
void describe('/rest/products/search', () => {
  ...
  void it('GET product search with quote and semicolon in criteria does not cause a SQL error', async () => {
    const res = await request(app)
  ...
  void it('GET product search treats UNION SELECT payloads as plain search criteria', async () => {
  ...
  void it('GET product search cannot leak user credentials via UNION SELECT', async () => {
    const res = await request(app)
