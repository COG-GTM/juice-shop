...
void describe('/rest/products/search', () => {
  ...
  void it('GET product search treats SQL metacharacters as literal search criteria', async () => {
    const res = await request(app)
  ...
  void it('GET product search treats UNION SELECT payload without closing parentheses as literal search criteria', async () => {
    const res = await request(app)
  ...
  void it('GET product search treats UNION SELECT payload with one closing parenthesis as literal search criteria', async () => {
    const res = await request(app)
      .get("/rest/products/search?q=') union select id,email,password from users--")
    assert.equal(res.status, 200)
    assert.ok(res.headers['content-type']?.includes('application/json'))
  ...
  void it('GET product search is not susceptible to UNION SELECT * FROM attack', async () => {
    const res = await request(app)
  ...
  void it('GET product search is not susceptible to UNION SELECT with fixed columns', async () => {
    const res = await request(app)
  ...
  void it('GET product search does not leak user credentials via UNION SELECT', async () => {
    ...
    assert.ok(!dataString.includes(`admin@${config.get<string>('application.domain')}`), 'Expected no user emails in search results')
    assert.ok(!dataString.includes(security.hash('admin123')), 'Expected no password hashes in search results')
  })

  void it('GET product search does not leak database schema via UNION SELECT', async () => {
    const res = await request(app)
    ...
    assert.equal(res.body.data.length, 0)
    const dataString = JSON.stringify(res.body.data)
    assert.ok(!dataString.includes('CREATE TABLE'), 'Expected no table definitions in search results')
  })
