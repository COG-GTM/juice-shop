...
void describe('/rest/products/search', () => {
  ...
  void it('GET product search treats a quote-terminating payload as a literal search term', async () => {
    const res = await request(app)
  ...
  void it('GET product search neutralizes a UNION SELECT injection with two missing closing parenthesis', async () => {
    const res = await request(app)
  ...
  void it('GET product search neutralizes a UNION SELECT injection with one missing closing parenthesis', async () => {
    const res = await request(app)
      .get("/rest/products/search?q=') union select id,email,password from users--")
    assert.equal(res.status, 200)
    assert.ok(res.headers['content-type']?.includes('application/json'))
  ...
  void it('GET product search neutralizes a SELECT * FROM UNION injection', async () => {
    const res = await request(app)
  ...
  void it('GET product search does not fabricate rows from a UNION SELECT with fixed columns', async () => {
    const res = await request(app)
  ...
  void it('GET product search does not leak the Users table via a UNION SELECT injection', async () => {
    const res = await request(app)
      .get("/rest/products/search?q=')) union select id,'2','3',email,password,'6','7','8','9' from users--")
    assert.equal(res.status, 200)
    assert.ok(res.headers['content-type']?.includes('application/json'))
    assert.equal(res.body.data.length, 0)
    assert.ok(!JSON.stringify(res.body).includes(`admin@${config.get<string>('application.domain')}`))
  })

  void it('GET product search does not leak the database schema via a UNION SELECT on sqlite_master', async () => {
    const res = await request(app)
