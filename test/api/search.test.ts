...
void describe('/rest/products/search', () => {
  ...
  void it('GET product search with a quote in the search term is treated as a literal', async () => {
    const res = await request(app)
  ...
  void it('GET product search UNION SELECT injection on Users table returns no data', async () => {
    const res = await request(app)
    ...
    const leakedAdmin = JSON.stringify(res.body).includes(security.hash('admin123'))
    assert.equal(leakedAdmin, false, 'Expected no user credentials in search results')
  })

  void it('GET product search UNION SELECT injection on sqlite_master returns no data', async () => {
    const res = await request(app)
      .get("/rest/products/search?q=')) union select sql,'2','3','4','5','6','7','8','9' from sqlite_master--")
    assert.equal(res.status, 200)
    assert.ok(res.headers['content-type']?.includes('application/json'))
    assert.equal(res.body.data.length, 0)
    assert.equal(JSON.stringify(res.body).includes('CREATE TABLE'), false, 'Expected no schema definitions in search results')
  })
