...
void describe('/api/Users', () => {
  ...
  void it('POST new user requesting admin role is registered as customer', async () => {
    const res = await request(app)
      .post('/api/Users')
      .set(jsonHeader)
      .send({
  ...
  void it('POST new user requesting deluxe token is registered without it', async () => {
    const res = await request(app)
      .post('/api/Users')
      .set(jsonHeader)
      .send({
        ...
        password: 'hooooorst',
        deluxeToken: 'i-am-deluxe',
        isActive: false
      })
    assert.equal(res.status, 201)
    assert.notEqual(res.body.data.deluxeToken, 'i-am-deluxe')
    assert.equal(res.body.data.role, 'customer')
  ...
  void it('POST new user requesting deluxe role is registered as customer', async () => {
    const res = await request(app)
      .post('/api/Users')
      .set(jsonHeader)
      .send({
  ...
  void it('POST new user requesting accounting role is registered as customer', async () => {
    const res = await request(app)
      .post('/api/Users')
      .set(jsonHeader)
      .send({
