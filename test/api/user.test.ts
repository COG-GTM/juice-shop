...
void describe('/rest/user/whoami', () => {
  ...
  void it('GET who-am-i with fields parameter ignores sensitive fields', async () => {
    const { token } = await login(app, {
      email: 'bjoern.kimminich@gmail.com',
      password: 'bW9jLmxpYW1nQGhjaW5pbW1pay5ucmVvamI='
    })
    const res = await request(app)
      .get('/rest/user/whoami?fields=id,email,password,totpSecret,role')
      .set({ Cookie: `token=${token}` })
    assert.equal(res.status, 200)
    assert.ok(res.headers['content-type']?.includes('application/json'))
  ...
  void it('GET who-am-i with only sensitive fields requested returns the default fields', async () => {
    const { token } = await login(app, {
      email: 'bjoern.kimminich@gmail.com',
      password: 'bW9jLmxpYW1nQGhjaW5pbW1pay5ucmVvamI='
    })
    const res = await request(app)
      ...
      .set({ Cookie: `token=${token}` })
    assert.equal(res.status, 200)
    assert.deepEqual(Object.keys(res.body.user).sort(), ['email', 'id', 'lastLoginIp', 'profileImage'])
  })
})
