...
void describe('/rest/products/reviews', () => {
  let reviewId: string
  let authorHeader: { Authorization: string }
  ...
  before(async () => {
    ...
    authorHeader = { Authorization: `Bearer ${token}` }
  })
  ...
  void it('PATCH product review of another author cannot be edited', async () => {
    const { token } = await login(app, {
  ...
  void it('PATCH multiple product reviews via injection is rejected', async () => {
    ...
    assert.ok(reviews.body.data.every(({ message }: { message: string }) => message !== 'trololololololololololololololololololololololololololol'))
  })
})
