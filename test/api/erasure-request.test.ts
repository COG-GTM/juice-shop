...
void describe('/dataerasure', () => {
  ...
  void it('POST erasure request with unknown layout parameter is rejected', async () => {
    const { token } = await login(app, { email: 'bjoern.kimminich@gmail.com', password: 'bW9jLmxpYW1nQGhjaW5pbW1pay5ucmVvamI=' })
  ...
  void it('POST erasure request with file path as layout parameter does not disclose file content', async () => {
    const { token } = await login(app, { email: 'bjoern.kimminich@gmail.com', password: 'bW9jLmxpYW1nQGhjaW5pbW1pay5ucmVvamI=' })
