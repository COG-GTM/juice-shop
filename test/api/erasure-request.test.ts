...
void describe('/dataerasure', () => {
  ...
  void it('POST erasure request with non-existing file path as layout parameter renders the result page', async () => {
    const { token } = await login(app, { email: 'bjoern.kimminich@gmail.com', password: 'bW9jLmxpYW1nQGhjaW5pbW1pay5ucmVvamI=' })
  ...
  void it('POST erasure request with existing file path as layout parameter does not disclose its content', async () => {
    const { token } = await login(app, { email: 'bjoern.kimminich@gmail.com', password: 'bW9jLmxpYW1nQGhjaW5pbW1pay5ucmVvamI=' })
