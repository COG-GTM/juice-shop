...
void describe('/ftp', () => {
  void it('GET directory listing without authentication is rejected', async () => {
    const res = await request(app)
      .get('/ftp')
    assert.equal(res.status, 401)
  })

  void it('GET a file without authentication is rejected', async () => {
    const res = await request(app)
      .get('/ftp/legal.md')
    assert.equal(res.status, 401)
  })

  void it('GET a file with an invalid token is rejected', async () => {
    const res = await request(app)
      .get('/ftp/legal.md')
  ...
  void it('GET a file authenticated through the session cookie', async () => {
    const res = await request(app)
      .get('/ftp/legal.md')
