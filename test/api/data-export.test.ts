...
async function requestCaptcha (authHeader: Record<string, string>) {
  ...
  const captcha = await ImageCaptchaModel.findOne({ order: [['id', 'DESC']] })
  assert.ok(captcha)
  return captcha.answer
}

void describe('/rest/user/data-export', () => {
  void it('Export data without use of CAPTCHA is rejected', async () => {
  ...
  void it('Export data using right answer to CAPTCHA', async () => {
    ...
    const answer = await requestCaptcha(authHeader)
  ...
  void it('Export data reusing an already solved CAPTCHA is rejected', async () => {
    const { token } = await login(app, { email: 'bjoern.kimminich@gmail.com', password: 'bW9jLmxpYW1nQGhjaW5pbW1pay5ucmVvamI=' })
    const authHeader = { Authorization: 'Bearer ' + token, 'content-type': 'application/json' }
