...
export class AboutComponent implements OnInit {
        ...
        for (let i = 0; i < feedbacks.length; i++) {
          this.galleryRef.addImage({
            ...
            args: {
              ...
              stars: AboutComponent.starsFor(feedbacks[i].rating)
            }
          })
        }
      })
  ...
  private static starsFor (rating: number): boolean[] {
    const filled = Math.min(Math.max(Math.trunc(rating) || 0, 0), AboutComponent.MAX_RATING)
    return Array.from({ length: AboutComponent.MAX_RATING }, (_, index) => index < filled)
  }
}
