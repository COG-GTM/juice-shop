...
export const isAuthorized = () => {
  const middleware = expressJwt(({ secret: publicKey }) as any)
...
export const verify = (token: string) => {
  ...
  try {
    return crypto.createVerify('RSA-SHA256').update(`${header}.${payload}`).verify(publicKey, Buffer.from(signature, 'base64url'))
  } catch {
    return false
  }
}
export const decode = (token: string) => { return jws.decode(token)?.payload }
export const verifyAndDecode = (token: string) => verify(token) ? decode(token) : undefined
