...
// The JWT signing key pair is never hardcoded. The private key is loaded from a
// secret manager / environment variable (JWT_PRIVATE_KEY) or a key file, and
// when none is configured a fresh pair is generated at startup and persisted to
// a local (git-ignored) key file so no key material has to ship with the source.
// The public key is always derived from the active private key so signing and
// verification stay in sync.
// Kept outside the publicly served encryptionkeys/ directory so the signing key
// can never be disclosed through file download or directory listing.
export const privateKeyFile = process.env.JWT_PRIVATE_KEY_FILE ?? 'jwt.key'

const loadOrCreateSigningKeys = (): { privateKey: string, publicKey: string } => {
  ...
  if (privateKeyPem) {
    ...
    try {
      ...
      /* read-only deployments keep the key in memory only */
    }
  }
