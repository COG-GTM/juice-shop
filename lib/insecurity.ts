...
export const publicKey = fs ? crypto.createPublicKey(privateKey).export({ type: 'spki', format: 'pem' }).toString() : 'placeholder-public-key'
...
if (fs) {
  try {
    ...
    logger.warn(`Could not update ${publicKeyFile} with the public key of the configured JWT key pair.`)
  }
}
