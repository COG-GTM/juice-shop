...
export const loadJwtPrivateKey = (keyFile: string): string => {
  ...
  const readUsableKey = (): string | undefined => {
    let contents: string
    try {
    ...
    try {
      crypto.createPrivateKey(contents)
    } catch {
      throw new Error(`JWT private key file "${keyFile}" exists but is not a usable private key. Remove it or set JWT_PRIVATE_KEY.`)
    }
    return contents
  }

  const existing = readUsableKey()
  if (existing) {
    return existing
  }
  ...
  try {
    ...
    try {
      ...
      return readUsableKey() ?? privateKey
    }
  } catch {
    return readUsableKey() ?? privateKey
  } finally {
