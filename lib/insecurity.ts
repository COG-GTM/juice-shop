...
// The HMAC of a security answer is the only thing protecting it, so its key must
// never be part of the source code. Without a configured secret a random one is
// generated per process, which keeps answers stored in this run verifiable while
// making precomputation impossible.
const loadHmacSecret = () => {
  const configuredSecret = secretFromEnvironment('HMAC_SECRET')
  if (configuredSecret !== undefined) {
    return configuredSecret
  }
  logger.warn('No HMAC_SECRET configured: generated an ephemeral HMAC secret, security answers will not verify across restarts')
  return crypto.randomBytes(32).toString('hex')
}

const hmacSecret = loadHmacSecret()
