...
const privateKeyPath = process.env.JWT_PRIVATE_KEY_FILE ?? 'encryptionkeys/jwt.key'
...
function loadOrCreatePrivateKey (): string {
  ...
  try {
    fs.writeFileSync(privateKeyPath, pem, { mode: 0o600, flag: 'wx' })
  } catch {
    try {
      return fs.readFileSync(privateKeyPath, 'utf8') // another process generated the key first
    } catch {
      // filesystem is not writable: keep the ephemeral in-memory key for this process
    }
  }
  return pem
}
